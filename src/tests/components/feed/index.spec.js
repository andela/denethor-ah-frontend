...
import FeedBottom from '../../../components/feed/index';
...
describe('Test for the feed bottom component', () => {
  ...
  const title = 'Introduction to writing'; 
  const createdAt = '2018-03-10';
  it('should contain FeedBottomLargeCard ', () => {
        const wrapper = shallow( < FeedBottomLargeCard  title={title} body={ body} author ={author } createdAt= {createdAt} /> );
    ...
    it('should contain FeedBottomSmallCard correctly', () => {
      const wrapper = shallow( < FeedBottomSmallCard title={title} body = { body } author ={author } createdAt= {createdAt} /> );
      expect(wrapper.length).toBe(1);
  ...
  it('should fail to dislike an article', () => {
		const getArticles = jest.fn(() => Promise.resolve({}));
		const props = {
      getArticles,
      articles: [],
      categories: [],
		};
		const wrapper = shallow( <FeedBottom {...props} /> );
